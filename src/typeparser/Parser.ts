...
import { type IntegerStrings, type Merge, type Trim } from "./Utils";
...
type ParseSelectStatement<T> = ParseSelectClause<T> extends Partial<
  SelectStatement<
    infer Fields,
    infer From,
    infer Joins,
...
type ParseTableSpecifier<T> = T extends `${infer Source} AS ${infer Alias}`
  ? TableSpecifier<Identifier<Source>, Identifier<Alias>>
  : T extends `${infer Source} ${infer Alias}`
  ? TableSpecifier<Identifier<Source>, Identifier<Alias>>
  : T extends string
  ? TableSpecifier<Identifier<Trim<T>>>
  : never;

type ParseSelectClause<T> =
  T extends `SELECT ${infer FieldNames} FROM ${infer R0}`
    ? Merge<
        { fields: ParseFieldSpecifierList<FieldNames> } & ParseFromClause<
          Trim<R0>
        >
      >
    : never;

type ParseFromClause<T> = Tokenize<T> extends [infer Source, infer R0]
    ...
    ? ParseExpression<R0> extends [infer Exp, infer R1]
      ? Exp extends Expression
        ? {
          ...
          } & ParseWhereClause<Trim<R1>>
        : never
      : never
    : ParseWhereClause<Trim<T>> & { joins: [] };

type ParseWhereClause<T> = Trim<T> extends ""
...
type ParseLimitClause<T> = Trim<T> extends `LIMIT ${infer R0}`
...
type ParseOffsetClause<T> = Trim<T> extends `OFFSET ${infer R0}`
...
type ParseStatementTerminator<T> = Trim<T> extends ""
  ? NonNullable<unknown>
  : Trim<T> extends ";"
  ? NonNullable<unknown>
  : never;
...
type ParseParenthesizedExpression<T> =
  T extends `(${infer Content})${infer Rest}`
    ? [ParseExpression<Content>, Rest]
    : never;

type ParseFieldSpecifierList<T> = T extends `${infer Head},${infer Tail}`
  ? [ParseFieldSpecifier<Trim<Head>>, ...ParseFieldSpecifierList<Trim<Tail>>]
  : T extends `${infer Head} AS ${infer Alias} ${infer Tail}`
  ? [
      FieldSpecifier<
        Trim<ParseMemberExpression<Head>[0]>,
        Trim<ParseIdentifier<Alias>[0]>
      >,
      Tail
    ]
  : T extends `${infer Head} AS ${infer Alias}`
  ? [
      FieldSpecifier<
        Trim<ParseMemberExpression<Head>[0]>,
        Trim<ParseIdentifier<Alias>[0]>
  ...
  ? [ParseFieldSpecifier<Trim<Head>>, Tail]
  : [ParseFieldSpecifier<Trim<T>>];

type ParseFieldSpecifier<T> = T extends `${infer Field} AS ${infer Alias}`
  ? FieldSpecifier<
