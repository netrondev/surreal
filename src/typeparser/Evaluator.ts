...
import { type MatchStringLike, type Merge } from "./Utils";
  ...
  ? From extends TableSpecifier<
    ...
    ? Source extends keyof DB
      ? Fields extends [FieldSpecifier<Identifier<"*">, Identifier<"*">>]
        ? DB[Source] //"a" //FilterWhere<CollectInputRows<DB, From, Joins>, Where>
        : DB[Source] // todo: filter out props
      : // : AssembleRows<
        //     Fields,
        //     ExtractFields<
...
type CollectJoinedArrayForRow<
  JoinTable extends readonly any[],
  JoinAlias extends string,
  TableRow,
  TableAlias extends string
> = {
  [P in keyof JoinTable]: Merge<
    TableRow & { [JA in JoinAlias]: JoinTable[P] } & {
...
type FirstElementOrNull<Table> = Table extends readonly any[]
  ? Table["length"] extends 0
    ? null
    : Table[0]
  : null;

type ExtractJoinAlias<Join> = Join extends InnerJoinSpecifier<
...
type AssembleEntries<Entries extends Iterable<readonly [PropertyKey, any]>> =
