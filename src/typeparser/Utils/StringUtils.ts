...
export type MatchStringLike<Left, Right> = Right extends `%${infer Content}%`
  ? MatchString<Left, Content>
  : Right extends `%${infer Content}`
  ? MatchStringEnd<Left, Content>
  : Right extends `${infer Content}%`
  ? MatchStringStart<Left, Content>
  : false;

export type EatFirstChar<T> = T extends `${infer _}${infer B}` ? B : "";

export type MatchStringStart<
  Candidate,
  Pattern extends string
> = Candidate extends Pattern
  ? true
  : Candidate extends `${Pattern}${infer _}`
  ? true
  : false;

export type MatchStringEnd<
  Candidate,
  Pattern extends string
> = Candidate extends Pattern
  ? true
...
export type Indent<T, Level = 1> = Level extends keyof Indents
